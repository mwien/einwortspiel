defmodule Einwortspiel.Game.RoundTest do
  ...
  test "round for two players" do
    ...
    assert {:error, :unauthorized_move} = Round.make_move(round, "Marcel", {:submit_guess, "Schmetterling"})
    assert {:ok, round} = Round.make_move(round, "Marcel", {:submit_clue, "Schmetterling"})
    assert Round.get_clue(round, "Marcel") == "Schmetterling"
    assert Round.get_phase(round) == :clues
    assert {:error, :unauthorized_move} = Round.make_move(round, "Marcel", {:submit_clue, "Schmetterling"})
    assert {:error, :unknown_move} = Round.make_move(round, "Marcel", {:submit_animal, "Schmetterling"})
    ...
    assert {:error, :unauthorized_move} = Round.make_move(round, "Marcel", {:submit_clue, Words.get_extraword(words, "Xyz")})
    assert {:ok, round} = Round.make_move(round, "Marcel", {:submit_guess, Words.get_extraword(words, "Marcel")})
    assert Round.get_phase(round) == :guesses
    assert {:ok, winning_round} = Round.make_move(round, "Clara", {:submit_guess, Words.get_extraword(words, "Clara")})
